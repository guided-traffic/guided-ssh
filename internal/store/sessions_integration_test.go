...
func TestHostSessionLifecycle(t *testing.T) {
	...
	// Create a host, user, CA key, and a certificate with a known serial.
	...
	// Session open with a serial ⇒ user_id is correlated.
	...
	if active[0].UserID == nil || *active[0].UserID != user.ID {
		t.Fatalf("serial correlation failed: user_id = %v", active[0].UserID)
	}
	if !active[0].StartedAt.Equal(started) {
		t.Errorf("StartedAt = %v, want %v", active[0].StartedAt, started)
	}

	// Session close closes exactly this session (host + user + tty).
	ended := now
	...
	mustNoErr(t, err)
	if len(active) != 0 {
		t.Fatalf("%d active sessions remain after close", len(active))
	}

	// Audit events: opened + closed written, closed with duration.
	...
	if types[store.EventSessionOpened] != 1 || types[store.EventSessionClosed] != 1 {
		t.Fatalf("audit events incomplete: %+v", types)
	}
}
...
func TestOpenHostSessionUnknownSerialTolerant(t *testing.T) {
	...
	if len(active) != 1 || active[0].UserID != nil {
		t.Fatalf("an unknown serial must tolerantly create the session without user_id: %+v", active)
	}
}
...
func TestRecordSudoEvent(t *testing.T) {
	...
	if got := string(events[0].Payload); !strings.Contains(got, "systemctl restart nginx") || !strings.Contains(got, "deploy") {
		t.Errorf("sudo payload incomplete: %s", got)
	}
}
