...
// fakeStore implements the Store interface in-memory; certificate and
// audit event land together only, just like in the real transaction.
...
func (f *fakeStore) ListActiveCAKeys(_ context.Context, purpose string) ([]store.CAKey, error) {
	var out []store.CAKey
	// Newest first, like the SQL implementation.
...
// AdoptCAKey mirrors the select-or-insert semantics of the real store
// (see store.AdoptCAKey): return an existing row unchanged, otherwise create
// a new one and set previously active keys of the same purpose to "retiring".
...
func TestNewWrongMasterKey(t *testing.T) {
	if _, err := New(&fakeStore{}, []byte("short"), NewPolicyEngine(DefaultPolicies())); !errors.Is(err, ErrInvalidMasterKey) {
...
func TestEnsureCAKeys(t *testing.T) {
	...
	if len(fs.keys) != 2 {
		t.Fatalf("expected 2 ca keys (user, host), got: %d", len(fs.keys))
	}
	if fs.keys[0].Purpose == fs.keys[1].Purpose {
		t.Fatal("expected separate keys for user and host")
	}
	// Idempotent: the second run creates nothing new.
	if err := c.EnsureCAKeys(ctx); err != nil {
		t.Fatalf("EnsureCAKeys (2nd run): %v", err)
	}
	if len(fs.keys) != 2 {
		t.Fatalf("EnsureCAKeys not idempotent: %d keys", len(fs.keys))
	}
	for _, et := range fs.eventTypes() {
		if et != EventKeyCreated {
			t.Fatalf("unexpected event %q", et)
		}
	}
}
...
func TestIssueUserCertificate(t *testing.T) {
	...
	if cert.Serial == 0 || int64(cert.Serial) != record.Serial { //nolint:gosec // test serial is small
		t.Fatalf("serial mismatch: cert=%d record=%d", cert.Serial, record.Serial)
	}
	...
	if record.UserID == nil || *record.UserID != userID {
		t.Fatal("UserID not carried over")
	}
	if !strings.Contains(string(record.IssuerContext), "sso-abc") {
		t.Fatalf("IssuerContext: %s", record.IssuerContext)
	}
	if len(fs.certs) != 1 || len(fs.events) != 3 { // 2x key_created + 1x cert_issued
		t.Fatalf("persistence: %d certificates, %d events", len(fs.certs), len(fs.events))
	}
	...
	// Signed by the user ca key, not the host key.
	...
	if record.CAKeyID != userCAKey.ID {
		t.Fatal("certificate not attributed to the user ca key")
	}
	...
	if !bytes.Equal(cert.SignatureKey.Marshal(), caPub.Marshal()) {
		t.Fatal("certificate not signed by the user ca key")
	}
}

func TestIssuePolicyViolation(t *testing.T) {
	c, fs := newTestCA(t)
	ctx := context.Background()
	if err := c.EnsureCAKeys(ctx); err != nil {
		t.Fatalf("EnsureCAKeys: %v", err)
	...
	if len(fs.certs) != 0 {
		t.Fatal("no certificate may be persisted on a policy violation")
	}
}

func TestIssueWithoutActiveKey(t *testing.T) {
	c, _ := newTestCA(t)
	if _, _, err := c.Issue(context.Background(), RequesterUser, userRequest(t), IssueRef{}); err == nil {
		t.Fatal("expected an error (no active ca key)")
	}
}

func TestIssuePersistenceErrorPropagated(t *testing.T) {
	c, fs := newTestCA(t)
	ctx := context.Background()
	if err := c.EnsureCAKeys(ctx); err != nil {
		t.Fatalf("EnsureCAKeys: %v", err)
	}
	fs.failCreateWithAudit = errors.New("db broken")
	if _, _, err := c.Issue(ctx, RequesterUser, userRequest(t), IssueRef{}); err == nil {
		t.Fatal("persistence error must propagate")
	}
}

func TestRotateAndBundle(t *testing.T) {
	c, fs := newTestCA(t)
	ctx := context.Background()
	if err := c.EnsureCAKeys(ctx); err != nil {
		t.Fatalf("EnsureCAKeys: %v", err)
	...
	if newKey.State != store.CAKeyStateActive {
		t.Fatalf("new key not active: %s", newKey.State)
	}
	if state := fs.keys[0].State; state != store.CAKeyStateRetiring {
		t.Fatalf("old key not retiring: %s", state)
	}

	// Transition window: the bundle contains both the old and the new key.
	bundle, err := c.Bundle(ctx, store.CertTypeUser)
	if err != nil {
		t.Fatalf("Bundle: %v", err)
	}
	if lines := strings.Split(strings.TrimSpace(bundle), "\n"); len(lines) != 2 {
		t.Fatalf("expected bundle with 2 keys, got %d:\n%s", len(lines), bundle)
	}
	if !strings.Contains(bundle, newKey.PublicKey) {
		t.Fatal("new key missing from the bundle")
	}

	// New certificates must come from the new key.
	...
	if record.CAKeyID != newKey.ID {
		t.Fatal("certificate after rotation not signed by the new key")
	}

	// Retire: the old key falls out of the bundle.
	...
	if lines := strings.Split(strings.TrimSpace(bundle), "\n"); len(lines) != 1 {
		t.Fatalf("expected bundle with 1 key:\n%s", bundle)
	}

	types := fs.eventTypes()
	if !slices.Contains(types, EventKeyRotated) || !slices.Contains(types, EventKeyRetired) {
		t.Fatalf("rotation/retire events missing: %v", types)
	}
}
